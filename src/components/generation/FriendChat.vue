<template>
  ...
  <el-breadcrumb>
    ...
    <el-breadcrumb-item>Friend Chat</el-breadcrumb-item>
  </el-breadcrumb>
  <!-- chat area -->
...
export default {
    ...
    methods: {
        ...
        async startChatStream() {
          ...
          const data = { 
            "chat_id": "chat_id",
            "agents": this.agent_val,
            "contents": this.contextFromMessages
          }
          // convert string
          ...
          // call API
          const response = await fetch('http://127.0.0.1:8000/generate/friendchat/stream', { 
            method: "POST",
          ...
          if (response.body) {
            ...
            // get all the stream data
            while (true) {
              const { done, value } = await reader.read();
              if (done) {
                break;
              }
              // if is waiting for response, clean the message
              if (this.messages[this.messages.length - 1].text == "Waiting for response...") {
                this.messages[this.messages.length - 1].text = "";
              }
              this.messages[this.messages.length - 1].text = this.messages[this.messages.length - 1].text + decoder.decode(value);
            }
            // clear input
            this.newMessage = "";
          }
