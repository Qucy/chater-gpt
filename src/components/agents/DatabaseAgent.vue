<template>
  ...
  <el-breadcrumb>
    ...
    <el-breadcrumb-item>Database agent</el-breadcrumb-item>
  </el-breadcrumb>
  <!-- chat area -->
...
export default {
    ...
    methods: {
        ...
        async startChatStream() {
          ...
          // call API          
          this.$axios.get('/agent/db/query/' + this.newMessage).then(response => {
            // clear input
            this.newMessage = "";
            // hanlde data
            ...
            // this.messages[this.messages.length - 1].text = response_chat.data.content
          })
          .catch(error => {
            console.log(error)
           })
